{
  ...
  engines =
    let
      mkEngine = { alias, iconUpdateURL ? "", params ? [ ], url }: {
        definedAliases = [ alias ];
        inherit iconUpdateURL;
        updateInterval = 56 * 24 * 60 * 60 * 1000; # every 8 weeks
        urls = [{ inherit params; template = url; }];
      };
    in
    {
    ...
    // {
      ...
      "Nix PR Tracker" = mkEngine {
        ...
        params = [{ name = "pr"; value = "{searchTerms}"; }];
        alias = "@pr";
      };
    ...
    # Miscellaneous
    // {
      "discord.py" = let version = "latest"; in mkEngine {
      ...
      "Library Genesis" = mkEngine {
        ...
        params = [{ name = "req"; value = "{searchTerms}"; }];
